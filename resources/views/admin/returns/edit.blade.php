...
<section>
    ...
    @endif
    <form id="myForm" method="post" action="{{ route('admin.returns.update',$id) }}">
    @csrf
        <div class="row">
            <div class="col-lg-12">
                <div class="row">
            ...
            <div class="table-responsive order-addmore" >
                <table class="table table-sm" id="timePriceTable">
                    ...
                    <tbody>
                        ...
                        @php
                            $getReturnDetails = getReturnDetails($id,$old_details[$key]['product_id']);
                            $isOld = 0;
                            $oldQty = 0;
                            $readonly = "";
                            if(!empty($getReturnDetails)){
                                $isOld = 1;
                                $oldQty = $getReturnDetails->quantity;
                                $readonly = "readonly";
                            }
                        @endphp
                        <tr id="tr_{{$key}}" class="tr_pro">
                            <td>
                                <input type="hidden" name="details[{{$key}}][isOld]" value="{{$isOld}}" id="isOld{{$key}}">
                                <input type="hidden" name="details[{{$key}}][oldQty]" value="{{$oldQty}}" id="oldQty{{$key}}">
                                <input type="hidden" name="details[{{$key}}][isNoCtnChanged]" value="0" id="isNoCtnChanged{{$key}}">
                                <input type="text" name="details[{{$key}}][product]"  placeholder="Search product by name ..." class="form-control select-md" {{$readonly}} id="product_name{{$key}}" onkeyup="getProductByName(this.value, {{$key}});" value="{{ old('details.'.$key.'.product') }}"> 
                        ...
                        @php
                            $getReturnDetails = getReturnDetails($id,$items->product_id);
                            $oldQty = $getReturnDetails->quantity;
                        @endphp
                        <tr id="tr_{{$i}}" class="tr_pro">
                            <td>
                                <input type="hidden" name="details[{{$i}}][isOld]" value="1" id="isOld{{$i}}">
                                <input type="hidden" name="details[{{$i}}][oldQty]" value="{{$oldQty}}" id="oldQty{{$i}}">
                                <input type="hidden" name="details[{{$i}}][isNoCtnChanged]" value="0" id="isNoCtnChanged{{$i}}">
                            ...
                            <td>
                                <input type="text" name="details[{{$i}}][hsn_code]" id="hsn_code{{$i}}" class="form-control" placeholder="Enter HSN Code" maxlength="6" value="{{$items->hsn_code}}">
                                
                            </td>
                            <td>
                                <input type="number"  name="details[{{$i}}][pcs]" id="pcs{{$i}}" class="form-control" value="{{$items->pcs}}" onkeyup="if(value<0) value=0;calculatePrice({{$i}})" onchange="if(value<0) value=0;calculatePrice({{$i}})">
                            </td>
                            ...
                            <td>
                                <input type="number"   name="details[{{$i}}][qty]" id="qty{{$i}}" min="1" class="form-control" placeholder="Product qty" value="{{$items->quantity}}" onkeyup="if(value<0) value=0;calculatePrice({{$i}})" onchange="if(value<0) value=0;calculatePrice({{$i}})">
                            </td>
                            <td>
...
<script>
    ...
    $(document).on('click','.addNewTime',function(){
        ...
        <tr id="tr_`+i+`" class="tr_pro">
            <td>
                <input type="hidden" name="details[`+i+`][isOld]" value="0" id="isOld`+i+`">
                <input type="hidden" name="details[`+i+`][oldQty]" value="0" id="oldQty`+i+`">
                <input type="hidden" name="details[`+i+`][isNoCtnChanged]" value="0" id="isNoCtnChanged`+i+`">
