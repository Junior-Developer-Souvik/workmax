...
@section('page', 'Edit Purchase Return')
@section('content')
<section>
    ...
    @endif --}}
    <form id="myForm" method="post" action="{{ route('admin.purchasereturn.update') }}">
    @csrf
        ...
        <div class="row">
            <div class="col-sm-4">
                ...
                <select class="form-control select-md"  name="supplier_id"  id="supplier_id">
                    ...
                    <option value="{{$item->id}}" @if($order->supplier_id == $item->id) selected @endif>{{ $item->name }}</option>
                    @endforeach
                </select> 
                        ...
                        <tbody>
                            ...
                            @else
                                ...
                                <tr id="tr_{{$i}}" class="tr_pro">
                                    <td>
                                        <input type="text" name="details[{{$i}}][product]"  placeholder="Search product by name ..." class="form-control select-md"  id="product_name{{$i}}" onkeyup="getProductByName(this.value, {{$i}});" value="{{$item->product->name}}"> 
                                    ...
                                    <td>
                                        <input type="number" name="details[{{$i}}][quantity]" id="quantity{{$i}}" class="form-control" placeholder="Enter Quantity" value="{{$item->quantity}}">
                                        
                                    </td>
                                    
                                    <td id="btn_td_{{$i}}">
                                        <a class="btn btn-sm btn-success actionTimebtn addNewTime" id="addNew{{$i}}">+</a>
                                        <a class="btn btn-sm btn-danger actionTimebtn removeTimePrice" onclick="removeRow({{$i}})" id="removeNew1">X</a>
                                    </td>
                                </tr>  
                                @php
                                    $i++;
        ...
        <div class="row">
            <div class="card shadow-sm">
                <div class="card-body">
                    @error('custom_err_msg1') <p class="small text-danger">{{ $message }}</p> @enderror 
