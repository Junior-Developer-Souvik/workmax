...
<body class="body" style="padding:0 !important; margin:0 !important; display:block !important; min-width:100% !important; width:100% !important; background:#f9f9f9; -webkit-text-size-adjust:none; font-family: Arial, Helvetica, sans-serif;">
	<table id="packing_table" style="width:700px; margin: 0 auto;" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td valign="top" style="font-size: 14px; font-weight: bold; padding: 10px; border-bottom: 1px solid #000;" width="70%">
                            {{$invoice->store->bussiness_name}}
                        </td>
						<td style="font-size: 14px; font-weight: normal; padding: 10px; border-left: 1px solid #000; border-bottom: 1px solid #000;" width="70%">{{$invoice->invoice_no}} <br> <br> {{ date('d/m/Y',strtotime($invoice->created_at)) }}</td>
					</tr>
				</table>
			</td>
		</tr>
		...
		@php
			$total_amount = 0;
		@endphp
		@foreach ( $invoice->products as $items )
		@php
			$total_amount += $items->total_price; 
		@endphp
		<tr>
			<td style="font-size: 13px; line-height: 1; font-weight: normal; color:#222; padding: 5px 10px; border-bottom: 1px solid #eee;" align="left">{{$items->product_name}}</td>
			<td style="font-size: 13px; line-height: 1; font-weight: normal; color:#222; padding: 5px 10px; border-bottom: 1px solid #eee;" align="center">{{$items->pcs}}</td>
			<td style="font-size: 13px; line-height: 1; font-weight: normal; color:#222; padding: 5px 10px; border-bottom: 1px solid #eee;" align="center">Rs. {{ number_format((float)$items->single_product_price, 2, '.', '') }}</td>
			...
			<td width="100%" valign="top" colspan="4" style="padding-top: 30px;">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="30%" style="padding: 10px 0; border-bottom: 1px solid #000; border-top: 1px solid #000; font-size: 14px;" align="right">Rs. {{ number_format((float)$total_amount, 2, '.', '') }}</td>
					</tr>
				</table>
			</td>
		</tr>
